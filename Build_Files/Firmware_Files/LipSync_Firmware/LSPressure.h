...
void LSPressure::begin()
{
  ...
  {
    Serial.println(F("ERROR: Couldn't find LPS35HW chip"));  // TODO this should throw an error
  ...
  else
  {
    ...
    if (USB_DEBUG)
    {
      Serial.println("USBDEBUG: LSPressure::begin LPS35HW Found.");
  ...
  {
    Serial.println(F("ERROR: Couldn't find LPS22 chip"));  // TODO this should throw an error
  ...
  else
  {
    ...
    if (USB_DEBUG)
    {
      Serial.println("USBDEBUG: LSPressure::begin LPS22 Found.");
...
void LSPressure::clear()
{
  ...
  {    
    pressureBuffer.pushElement({0.0, 0.0, 0.0});  // Clear pressureBuffer by pushing zero readings to buffers 
  }
...
float LSPressure::getOffsetPressure()
{
  ...
  else if(pressureMode == PRESS_MODE_ABS){// If pressure mode is absolute 
...
void LSPressure::updateState()
{
  ...
  if (pressureValue > puffThreshold) {
    ...
    sapMainState = PRESS_SAP_MAIN_STATE_SIP; // Sip detected
  }
